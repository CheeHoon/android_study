...
public class Ex22SQLiteExLogin extends AppCompatActivity {

    EditText etId, etPw;

    //SQLITE 데이타베이스 관련변수
    SQLiteDatabase db;
    com.sch.polio.MySQLiteOpenHelper helper;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.layout_ex22_sqlite_login);

        //데이베이스 생성.
        helper = new com.sch.polio.MySQLiteOpenHelper(
                Ex22SQLiteExLogin.this, // 현재 화면의 context
                "member2.db", // 파일명
                null, // 커서 팩토리
                1); // 버전 번호
        ...
        public void onClick(View v)
        {
            switch(v.getId())
            {
                case R.id.btnLogin:
                    ...
                    if(loginCheck ==true)
                    {
                        Intent login = new Intent(Ex22SQLiteExLogin.this, com.sch.polio.Ex22SQLiteExMain.class);
                        startActivity(login);
                    }
                    else
                    {
                        Toast.makeText(Ex22SQLiteExLogin.this, "로그인 실패!(아이디/패스워드확인후다시하세요)",  Toast.LENGTH_SHORT).show();
                    }

                    break;
                case R.id.btnJoin:
                    Intent login = new Intent(Ex22SQLiteExLogin.this, com.sch.polio.Ex22SQLiteExJoin.class);
                    startActivity(login);
                    break;
            }
